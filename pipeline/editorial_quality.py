...
_CHAPTER_CONTEXT = {
    "top-headlines": ("국내외 주요 현안의 후속 사실관계와 공식 대응을 확인해야 하는 사안입니다.", "정책·여론·시장 반응이 어떻게 이어지는지 확인할 필요가 있습니다.", "정부·관계기관의 공식 후속 발표"),
    "politics-policy": ("정책 결정과 입법·행정 절차의 후속 변화가 핵심입니다.", "실제 제도 변경 여부와 이해관계자 영향 범위를 확인할 필요가 있습니다.", "정부·국회·관계기관의 공식 절차와 후속 일정"),
    "macro-finance": ("금리·환율·증시·유동성 등 금융 변수의 변화를 함께 봐야 하는 사안입니다.", "시장 가격과 자금 흐름에 미치는 실제 영향을 후속 지표로 확인할 필요가 있습니다.", "금리·환율·수급 등 핵심 시장지표의 후속 변화"),
    "global-affairs": ("외교·안보·통상 환경의 변화와 당사국의 후속 대응을 함께 확인해야 하는 사안입니다.", "지역 안보와 국제관계에 미치는 영향은 공식 조치와 후속 협상에 따라 달라질 수 있습니다.", "당사국 정부와 국제기구의 공식 후속 조치"),
    "tech-it": ("제품·플랫폼·규제·보안 등 기술 산업의 실제 변화 여부를 확인해야 하는 사안입니다.", "이용자·기업·생태계에 미치는 영향은 출시·도입·규제 집행 결과로 확인할 필요가 있습니다.", "제품 출시·서비스 적용·규제 집행의 실제 일정"),
    "ai-deeptech": ("AI 모델·인프라·정책의 실제 성능과 적용 범위를 확인해야 하는 사안입니다.", "기술 경쟁력과 산업 파급력은 검증된 성능·비용·도입 사례를 통해 확인할 필요가 있습니다.", "모델 성능·가격·도입 사례와 공식 기술 문서"),
    "semiconductors-mfg": ("반도체 수요·생산·투자·공급망의 실제 변화를 확인해야 하는 사안입니다.", "산업 영향은 수율·출하·고객사·투자 집행 등 후속 실적에서 확인할 필요가 있습니다.", "생산·출하·수율·투자 집행 관련 후속 지표"),
    "bio-healthcare": ("임상·허가·사업화·보건정책의 공식 근거를 확인해야 하는 사안입니다.", "환자·의료기관·기업에 미치는 영향은 임상 결과와 허가·정책 결정에 따라 달라질 수 있습니다.", "규제기관·학회·기업의 공식 임상·허가 발표"),
    "energy-mobility": ("에너지 가격·공급·설비와 모빌리티 정책·수요 변화를 함께 봐야 하는 사안입니다.", "실제 영향은 공급량·가격·판매·인프라 지표의 후속 변화로 확인할 필요가 있습니다.", "에너지 수급·가격 및 차량·인프라 후속 지표"),
    "realestate-construction": ("주택 공급·가격·금융·건설 사업의 실제 변화를 확인해야 하는 사안입니다.", "가계와 건설업계 영향은 거래·분양·대출·PF 관련 후속 지표로 확인할 필요가 있습니다.", "거래·분양·대출·PF 관련 공식 통계와 후속 조치"),
    "retail-consumer": ("가격·판매·유통채널·소비자 반응의 변화를 확인해야 하는 사안입니다.", "내수와 기업 실적 영향은 판매량·객단가·소비지표의 후속 변화로 확인할 필요가 있습니다.", "판매·가격·소비심리 관련 후속 지표"),
    "society-environment": ("노동·인구·환경·안전 등 사회적 영향과 제도적 대응을 확인해야 하는 사안입니다.", "영향 범위는 공식 통계와 정책 집행, 현장 변화에 따라 달라질 수 있습니다.", "정부·지자체·관계기관의 공식 통계와 후속 대책"),
    "culture-entertainment": ("콘텐츠 성과·시장 반응·권리 관계의 실제 변화를 확인해야 하는 사안입니다.", "산업 영향은 흥행·판매·시청·계약 등 검증 가능한 후속 성과로 확인할 필요가 있습니다.", "흥행·판매·시청·계약 관련 후속 성과"),
    "science-future": ("연구 결과·실증·개발 일정의 검증 수준을 확인해야 하는 사안입니다.", "의미는 재현성·공식 데이터·후속 연구 및 실제 적용 가능성에 따라 달라질 수 있습니다.", "논문·기관 발표·실증 결과 등 공식 근거"),
}
_PARTICLE_SUFFIXES = ("에서", "으로", "에게", "까지", "부터", "한테", "은", "는", "을", "를", "에", "로", "와", "과", "의", "도", "만")
_BAD_PATTERNS = ("을(를)", "이(가)", "은(는)", "에에 직접", "와와 직접", "과과 직접", "로로 전해졌", "경신로 전해졌", "핵심 확인 키워드는")
...
def _clean_display_title(title: str, source: str) -> str:
    clean = _without_headline_tags(title)
    forms = {_normalize(source), _editorial_source(source)} - {"", "원문 매체", "주요 언론"}
    for form in sorted(forms, key=len, reverse=True):
        clean = re.sub(rf"\s*[-–—|:]\s*{re.escape(form)}\s*$", "", clean, flags=re.I)
    return _normalize(clean) or _normalize(title)


def _looks_like_noise(fragment: str, title: str, source: str) -> bool:
    f = fragment.lower()
    if not fragment or fragment == title or fragment == source or any(domain in f for domain in _NOISE_DOMAINS):
        return True
...
def _same_event_clause(fragment: str, title: str) -> bool:
    a, b = informative_title_tokens(title), informative_title_tokens(fragment)
    if not a or not b:
        return False
    shared = a & b
    if len(shared) < 2:
        return False
    return len(shared) / max(1, min(len(a), len(b))) >= 0.50 and len(shared) / max(1, len(a | b)) >= 0.25


def _publisher_hits(fragment: str, selected_source: str) -> List[str]:
    low, selected = fragment.lower(), _editorial_source(selected_source).lower()
...
def _strip_selected_title_and_source(piece: str, title: str, source: str) -> str:
    clean_title = _clean_display_title(title, source)
...
def _summary_candidates(summary: str, title: str, source: str) -> List[str]:
    ...
    for raw_piece in pieces:
        piece = _strip_selected_title_and_source(_normalize(re.sub(r"\[[^\]]{1,30}\]", " ", raw_piece)), title, source)
        if len(piece) < 12 or _looks_like_noise(piece, title, source):
            continue
        if any(domain in piece.lower() for domain in _NOISE_DOMAINS) or _publisher_hits(piece, source) or not _same_event_clause(piece, title):
            continue
        candidates.append(piece[:240].rstrip(" ,;"))
    return candidates


def _fallback_fact(source: str, title: str, raw_source: str) -> str:
    clean_title = _clean_display_title(title, raw_source)
...
def _fact_text(raw: Dict[str, Any]) -> str:
    ...
    return _fallback_fact(source, title, raw_source)
...
def _clean_keywords(title: str, summary: str, chapter_id: str) -> List[str]:
    merged: List[str] = []
    for word in [*extract_keywords(title, "", chapter_id), *extract_keywords(title, summary, chapter_id)]:
...
def _semantic_gate(editorial: EditorialContent) -> None:
...
def build_editorial_for_article_v2(raw: Dict[str, Any]) -> EditorialContent:
    ...
    focus = _clean_display_title(title, raw_source)[:90]

    fact = _fact_text(raw)
    background = f"‘{focus}’ 관련 보도입니다. {context[0]} 현재 {source} 등 주요 매체의 후속 보도를 함께 확인할 필요가 있습니다."
    why = f"‘{focus}’의 의미는 후속 사실관계와 공식 대응에 따라 달라질 수 있습니다. {context[1]}"
    ...
    evidence_checkpoint = f"독립 근거 도메인({', '.join(evidence_domains[:3])})의 후속 보도 일치 여부" if len(evidence_domains) >= 2 else "독립된 두 번째 근거 또는 공식 1차 자료의 추가 확인"
