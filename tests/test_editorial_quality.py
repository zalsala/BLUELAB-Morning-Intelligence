...
def test_fact_rejects_google_news_or_daum_relay_fragment():
    editorial = build_editorial_for_article_v2(_article(summary_raw="연합뉴스 이 시각 헤드라인 - 10:30 v.daum.net [속보] 다른 사건 연합뉴스"))
...
def test_fact_rejects_unrelated_concatenated_headline_without_domain_marker():
    editorial = build_editorial_for_article_v2(_article(summary_raw="7월 경상수지 421억달러 흑자…반도체 호황에 역대 최대 한국경제 대한민국 정책브리핑 대한민국의 미래, 청년 과학기술인을 응원한다 안도걸 의원, 2027년도 예산안 관련 기자회견 뉴시스"))
...
def test_fact_requires_same_event_overlap_not_just_long_fragment():
    editorial = build_editorial_for_article_v2(_article(summary_raw="정부가 로봇 200대를 구매하는 정책을 검토하면서 관련 기업 주가가 상승했다는 분석이 나왔다."))
    assert "로봇 200대" not in editorial.fact
    assert "세부 사실관계는 원문과 추가 근거에서 확인해야 합니다" in editorial.fact


def test_fact_rejects_same_event_multi_publisher_cluster():
    editorial = build_editorial_for_article_v2(_article(title="월러 연준 이사 발언에 코스피 상승", source="MBC 뉴스", summary_raw="월러 연준 이사 발언에 코스피 상승 MBC 뉴스 월러 연준 이사 금리 인하 가능성 언급 뉴시스 코스피 장중 상승폭 확대 아시아경제"))
...
def test_fact_rejects_neighboring_sub_event_with_partial_token_overlap():
    editorial = build_editorial_for_article_v2(_article(chapter_id="global-affairs", title='이란, 쿠웨이트 미군기지 공격…"이란 국민 상대 범죄 대응"', source="MBC 뉴스", summary_raw='불바다 된 이란 “보복작전 명칭은 번개…UAE·바레인 미군 기지 드론 공격”'))
...
def test_known_domain_source_is_rendered_as_publisher_name_without_particle_error():
    editorial = build_editorial_for_article_v2(_article(source="yna.co.kr", title="7월 경상수지 420.8억달러 흑자…역대 두 번째 규모", summary_raw=""))
...
def test_headline_echo_with_breaking_tag_and_source_falls_back():
    editorial = build_editorial_for_article_v2(_article(title="[속보] 7월 경상수지 421억달러 흑자…반도체 호황에 동월 역대 최대", summary_raw="[속보] 7월 경상수지 421억달러 흑자…반도체 호황에 동월 역대 최대 한국경제"))
    assert "한국경제로 전해졌습니다" not in editorial.fact
    assert "세부 사실관계는 원문과 추가 근거에서 확인해야 합니다" in editorial.fact


def test_production_regression_khan_source_never_becomes_newspaper_neun():
    editorial = build_editorial_for_article_v2(_article(chapter_id="top-headlines", title="[속보]이 대통령 지지율 40%, 2%P↓···최저치 경신[한국갤럽]", source="경향신문", summary_raw="이 대통령 지지율 40%, 2%P↓···최저치 경신"))
    assert "경향신문는" not in editorial.fact
    assert "경신로 전해졌습니다" not in editorial.fact
    assert editorial.fact.startswith("경향신문 보도에 따르면")


def test_production_regression_morphology_sensitive_keywords_are_not_rendered():
    editorial = build_editorial_for_article_v2(_article(title="李대통령 경찰 뼈를 깎는 각오로 자정 나서야", source="연합뉴스", summary_raw=""))
    assert "핵심 확인 키워드는" not in editorial.why_it_matters
    assert "뼈를입니다" not in editorial.why_it_matters
    assert "李대통령" in editorial.why_it_matters


def test_production_regression_trailing_publisher_is_removed_from_display_title():
    editorial = build_editorial_for_article_v2(_article(chapter_id="tech-it", title="카카오멤버십 기본구독료 4900원…네이버와 정면승부 - 머니투데이", source="머니투데이", summary_raw=""))
    assert "정면승부 - 머니투데이’" not in editorial.fact
    assert "정면승부’이라는 내용" in editorial.fact


def test_global_affairs_does_not_force_korean_export_template():
    editorial = build_editorial_for_article_v2(_article(chapter_id="global-affairs", title="이스라엘 안보 장관 가자 주민 이주 계획 언급", source="한겨레", summary_raw=""))
...
def test_relay_domains_are_not_exposed_as_independent_checkpoint_evidence():
    editorial = build_editorial_for_article_v2(_article(fact_check={"evidence_urls": ["https://www.hankyung.com/article/1", "https://v.daum.net/v/123", "https://news.nate.com/view/456"]}))
...
def test_background_and_why_are_article_specific():
    ...
    assert "7월 경상수지" in editorial.why_it_matters
