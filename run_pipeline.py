...
def main():
    ...
    try:
        ...
        bundle.metadata["general_chapters"]=len(bundle.chapters)-1; bundle.metadata["general_articles"]=len(articles)
        ...
        content_counts={"total_chapters":len(bundle.chapters),"total_articles":len(articles)+10,"general_chapters":len(bundle.chapters)-1,"general_articles":len(articles),"vision_research_watch":10,"top5":len(bundle.top_5_highlights),"youtube":len(bundle.youtube_hot_issues),"trends":len(bundle.trending_keywords),"summary_lines":len(bundle.three_line_summary),"story_bundles":len(story_files)}
        ...
        print("\n"+"*"*80); print("  PIPELINE CANONICAL PASS — ALL CORE GATES PASSED"); print(f"  일반 15개/150기사 + Vision Research Watch 10건 / Story bundles {len(story_files)} / YouTube {len(bundle.youtube_hot_issues)} / Trends {len(bundle.trending_keywords)}"); print(f"  소요 시간: {elapsed:.2f}초 | 저장 위치: {today_path}"); print(f"  발간 매니페스트 SHA-256: {manifest['manifest_sha256']}"); print("*"*80+"\n")
